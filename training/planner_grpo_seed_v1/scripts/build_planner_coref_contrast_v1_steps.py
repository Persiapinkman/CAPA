#!/usr/bin/env python3
"""Build the preregistered coreference-focused GRPO training view."""
...
DEFAULT_OUTPUT = (
    ROOT / "training/planner_grpo_seed_v1/sft_data_coref_contrast_v1_chatml/train.jsonl"
)

# The target is oversampled, while four state/action contrasts prevent a
# shortcut that emits re_question for every retrieval-related prompt.
SELECTION_WEIGHTS = {
    ("coref_rewrite_then_rag", 1): 4,
    ("rag_miss_rewrite_then_rag", 1): 1,
...
def main() -> None:
    ...
    if len(rows) != 192:
        raise ValueError(f"expected 192 weighted rows, got {len(rows)}")
    ...
    metadata = {
        "schema_version": "1.0",
        "dataset_id": "planner_coref_contrast_v1",
        "source": str(source),
        ...
        "weighted_categories": dict(sorted(Counter(str(row["category"]) for row in rows).items())),
    }
