#!/usr/bin/env python3
"""Freeze the V10 nine-scenario anti-forgetting optimizer dataset."""
...
EXPECTED_SCOPES = {"primary_residual", "stability_control"}
...
def main() -> None:
    ...
    if accepted != EXPECTED_SCENARIOS:
        raise ValueError("V10 optimizer scope is not the preregistered all-or-none set")
    manifest = freeze_optimizer_data(
        ...
        allowed_optimization_scopes=EXPECTED_SCOPES,
    )
    if (
        set(manifest["accepted_scenarios"]) != EXPECTED_SCENARIOS
        or set(manifest["allowed_optimization_scopes"]) != EXPECTED_SCOPES
        or manifest["rows"] != 432
    ):
        raise ValueError("V10 optimizer manifest violates the preregistered scope")
