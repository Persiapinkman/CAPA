#!/usr/bin/env python3
"""Freeze the V11 1:1 non-migration/migration optimizer dataset."""
...
def main() -> None:
    ...
    if accepted != EXPECTED_SCENARIOS:
        raise ValueError("V11 optimizer scope is not the preregistered all-or-none set")
    ...
    if (
        ...
        raise ValueError(
            f"V11 optimizer manifest violates action balance: rows={manifest['rows']} "
            f"actions={actions}"
        )
