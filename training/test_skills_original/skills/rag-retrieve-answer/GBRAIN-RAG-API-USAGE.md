...
| `knowledge_base_fully_answered` | `number`（0~1）或 `boolean`（兼容） | 知识库对本次问题的覆盖置信度；Demo 下游将 `< 0.97` 视为未充分命中（等同旧版 `false`），`>= 0.97` 视为命中（等同旧版 `true`）。缺失或无法解析按 `0` 处理 |
...
## Demo 服务集成（`projects/test-skills/demo`）

`demo_server.py` 通过 `gbrain_rag_client.py` 支持两种 RAG 调用方式，由环境变量切换：

| 环境变量 | 默认值 | 说明 |
| --- | --- | --- |
| `DEMO_RAG_API_MODE` | `playbook` | `playbook`：现有 Playbook 增强问答（6062）；`unified`：gbrain-rag unified API（6061） |
| `GBRAIN_RAG_BASE_URL` | `http://127.0.0.1:6061/api/v1/rag` | unified 模式下的服务根路径 |
| `RAG_UNIFIED_RETRIEVE_URL` / `RAG_UNIFIED_QUERY_URL` | - | 可选，覆盖 unified 检索/问答完整 URL |
| `RAG_BASE_URL` / `RAG_QUERY_URL` | `http://127.0.0.1:6062/api/v1/playbook/query` | playbook 模式问答地址 |
| `DEMO_RAG_INCLUDE_FULL_DOCUMENTS` | `1` | unified 检索与 playbook retrieve 是否带 `include_full_documents=true` |
| `RAG_TOP_K` | `12` | 返回证据数量上限 |
| `RAG_RETRIEVAL_METHOD` | `hybrid` | unified 检索方法 |

示例（使用 unified + 完整文档）：

```bash
export DEMO_RAG_API_MODE=unified
export DEMO_RAG_INCLUDE_FULL_DOCUMENTS=1
export GBRAIN_RAG_BASE_URL=http://127.0.0.1:6061/api/v1/rag
python3 demo_server.py --port 18080
```

迁移顾问 Playbook 字段检索与 Agent 多轮 RAG 问答共用上述开关；修改后需重启 `demo_server`。

### 在 `demo_server.py` 里用代码配置（推荐本地开发）

在 `demo_server.py` 顶部有 `DEMO_RAG_*` 常量块，导入后会自动调用 `gbrain_rag.configure_rag()`。例如改用 unified：

```python
DEMO_RAG_API_MODE = "unified"
DEMO_RAG_GBRAIN_BASE_URL = "http://127.0.0.1:6061/api/v1/rag"
DEMO_RAG_INCLUDE_FULL_DOCUMENTS = True
```
...
gbrain_rag.configure_rag(
    api_mode="unified",
    include_full_documents=True,
    sync_to_environ=True,  # 可选：同步到 os.environ 供子进程读取
)
```

配置优先级：`configure_rag()` 参数 > 环境变量 > 内置默认值。
