...
from training.planner_grpo_seed_v1.scripts.audit_qwen35_v12_35b_stability_n10_early_stop import (
    audit_early_stop,
)
...
def test_early_stop_audit_preserves_partial_denominator_and_proves_negative_bound(
    tmp_path,
):
    cases = []
    for index in range(96):
    ...
    for index in range(10):
        row = prediction(f"c{index:03d}")
        row["decisions"][0]["action"] = "answerer"
        row["decisions"][0]["decision_type"] = "tool"
        row["decisions"][0]["_planner_metrics"] = {
            "error_type": "ValueError",
            "error": "planner schema parse failed after retry",
        }
        failure_rows.append(row)
    raw = tmp_path / "raw"
    write_jsonl(raw / "shard0" / "p_shard0_run1_predictions.jsonl", failure_rows)
    checkpoint = {
        "run": 1,
        "expected_runtime_error_rows": 2,
        "expected_transport_failure_rows": 0,
        "expected_strict_passes": 0,
        "case_ids": ["c000", "c001"],
    }
    ...
    report = audit_early_stop(
        ...
        raw_root=raw,
        analysis_dir=tmp_path / "early",
        stop_reason="fixture irreversible gate failure",
    )
    assert report["hard_success_possible_after_stop"] is False
    assert report["raw_retry_accounting"] == {
        "expected_primary_rows": 288,
        "observed_primary_rows": 10,
        "unobserved_primary_rows": 278,
        "retry_rows": 0,
        "silent_expected_decision_fill_used": False,
    }
    assert report["partial_run_tables"]["1"]["weighted_rate_not_reported"] is True
    assert report["optimistic_completion_upper_bound"]["dev_score_upper_bound_pp"] < 0
    assert report["optimistic_completion_upper_bound"]["is_nonpositive"] is True
