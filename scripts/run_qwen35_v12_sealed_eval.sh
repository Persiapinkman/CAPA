...
OUT_DIR="${OUT_DIR:-experiments/runs/20260717_qwen35_v12_sealed}"
...
[[ "$(jq -r '.status' "${OPENING}")" == "sealed_test_materialized_once" ]] || {
  echo "V12 sealed test has not been validly opened" >&2
  exit 1
}
...
for shard in 0 1 2; do
  ...
  for family in sft grpo; do
    ...
    CUDA_VISIBLE_DEVICES="${gpu}" "${PYTHON_BIN}" \
      ...
      --report-prefix "v12_sealed_${family}_shard${shard}" \
...
REPORT_PREFIX="v12_sealed_qwen35_35b_reference_t0" \
...
for family in sft grpo; do
  predictions=()
  for shard in 0 1 2; do
    predictions+=("${OUT_DIR}/${family}/shard${shard}/v12_sealed_${family}_shard${shard}_run1_predictions.jsonl")
  done
