...
case "${RUN_MODE}" in
  canary)
    ...
    OUTPUT_DIR="${OUTPUT_DIR:-experiments/runs/${RUN_TIMESTAMP}_qwen35_4b_optimizer_matched_v12_canary2_seed42}"
    ;;
  screen)
    MAX_STEPS="${MAX_STEPS:-8}"
    SAVE_STEPS="${SAVE_STEPS:-1}"
    SAVE_TOTAL_LIMIT="${SAVE_TOTAL_LIMIT:-10}"
    OUTPUT_DIR="${OUTPUT_DIR:-experiments/runs/${RUN_TIMESTAMP}_qwen35_4b_optimizer_matched_v12_screen8_seed42}"
    ;;
  *)
...
[[ -f "${ADAPTER_PATH}/adapter_model.safetensors" ]] || { echo "Missing V12 initializer" >&2; exit 1; }
[[ "$(sha256sum "${ADAPTER_PATH}/adapter_model.safetensors" | awk '{print $1}')" == "${EXPECTED_ADAPTER_SHA}" ]] || {
  echo "V12 continuation initializer SHA-256 changed" >&2
  exit 1
}
...
if decision.get("status") != "pass" or decision.get("optimizer_authorized") is not True:
    raise SystemExit("V12 support gate does not authorize optimizer steps")
if set(decision.get("optimizer_scenarios") or []) != expected:
    raise SystemExit("V12 support decision does not authorize all nine scenarios")
if any(not item.get("passed") for item in decision.get("hard_checks") or []):
    raise SystemExit("V12 support decision contains a failed hard check")
if manifest.get("dataset_id") != "planner_retry_optimizer_matched_v12":
    raise SystemExit("V12 optimizer manifest dataset changed")
if manifest.get("rows") != 576 or set(manifest.get("accepted_scenarios") or []) != expected:
    raise SystemExit("V12 optimizer manifest does not match preregistration")
if manifest.get("scenario_multipliers") != {
    **{scenario: 2 for scenario in primary},
    **{scenario: 1 for scenario in controls},
}:
    raise SystemExit("V12 optimizer primary replay multipliers changed")
if manifest.get("distribution", {}).get("target_actions") != {
    "end": 192, "migrate": 288, "retry": 96
}:
    raise SystemExit("V12 optimizer action balance changed")
PY

export RUN_MODE MAX_STEPS SAVE_STEPS SAVE_TOTAL_LIMIT OUTPUT_DIR STEP_DATA ADAPTER_PATH CUDA_VISIBLE_DEVICES
export EXPECTED_DATASET_ID="planner_retry_optimizer_matched_v12"
...
export WANDB_RUN_GROUP="planner-retry-optimizer-matched-v12"
export WANDB_TAGS="stage2,grpo,planner-v12,safety-weighted,optimizer-matched,continuation,${RUN_MODE},seed42"
