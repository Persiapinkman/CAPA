...
RUN_DIR="${RUN_DIR:-experiments/runs/20260717_qwen35_4b_v11_support4x_v10ckpt10}"
STUDY_DIR="experiments/studies/planner_retry_safety_balanced_v11_qwen35_4b_v1"
DATASET_DIR="data/datasets/planner_retry_safety_balanced_v11"
STEP_DATA="training/planner_grpo_seed_v1/step_data/planner_retry_safety_balanced_v11_support_dev_qwen35_4b_nothinking_mixed_steps.jsonl"
...
[[ "$(sha256sum "${ADAPTER}/adapter_model.safetensors" | awk '{print $1}')" == "${EXPECTED_ADAPTER_SHA}" ]] || {
  echo "V11 continuation initializer hash changed" >&2
  exit 1
}
...
[[ "${status}" -eq 0 ]] || { echo "V11 support sampling failed" >&2; exit 1; }
...
"${PYTHON_BIN}" training/planner_grpo_seed_v1/scripts/gate_planner_retry_safety_balanced_v11_support.py \
