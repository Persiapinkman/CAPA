...
ENV_DIR="${ENV_DIR:-${ROOT_DIR}/.venv-qwen35-math}"
...
DATA_DIR="${DATA_DIR:-training/public_sft_grpo_v1/data/math_sft1024_v1}"
...
case "${RUN_MODE}" in
  dry-run)
    TRAIN_MODE="dry-run"
    OUTPUT_DIR="${OUTPUT_DIR:-experiments/runs/20260715_qwen35_4b_math_sft1024_dryrun_v1}"
    ;;
  train)
    TRAIN_MODE="train"
    OUTPUT_DIR="${OUTPUT_DIR:-experiments/runs/20260715_qwen35_4b_math_sft1024_screen_v1}"
    ;;
  *)
...
if [[ ! -x "${PYTHON_BIN}" || ! -x "${TORCHRUN_BIN}" ]]; then
  echo "Missing frozen Qwen3.5 Math environment under ${ENV_DIR}" >&2
  exit 1
fi
if [[ ! -d "${MODEL_PATH}" || ! -f "${DATA_DIR}/manifest.json" ]]; then
  echo "Missing model or prepared MATH data" >&2
  exit 1
fi

IFS=',' read -r -a GPU_IDS <<< "${CUDA_VISIBLE_DEVICES}"
if [[ "${#GPU_IDS[@]}" -ne "${NUM_PROCESSES}" || "${NUM_PROCESSES}" -ne 4 ]]; then
  echo "MATH SFT1024 v1 requires exactly four visible GPUs and four ranks" >&2
  exit 1
fi
...
if [[ "${RUN_MODE}" == "dry-run" ]]; then
  "${PYTHON_BIN}" training/public_sft_grpo_v1/scripts/train_qwen35_math_sft.py "${common_args[@]}"
else
  "${TORCHRUN_BIN}" --standalone --nnodes=1 --nproc-per-node="${NUM_PROCESSES}" \
    training/public_sft_grpo_v1/scripts/train_qwen35_math_sft.py "${common_args[@]}"
fi
