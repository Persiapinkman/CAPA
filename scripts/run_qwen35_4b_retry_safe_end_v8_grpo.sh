...
SUPPORT_DECISION="${SUPPORT_DECISION:-experiments/studies/planner_retry_migrate_residual_v8_qwen35_4b_v1/support_decision.json}"
STEP_DATA="${STEP_DATA:-training/planner_grpo_seed_v1/step_data/planner_retry_safe_end_residual_v8_optimizer_qwen35_4b_nothinking_mixed_steps.jsonl}"
...
case "${RUN_MODE}" in
  canary)
    ...
    OUTPUT_DIR="${OUTPUT_DIR:-experiments/runs/${RUN_TIMESTAMP}_qwen35_4b_retry_safe_end_v8_canary5_seed42}"
    ;;
  screen)
    MAX_STEPS="${MAX_STEPS:-40}"
    SAVE_STEPS="${SAVE_STEPS:-10}"
    OUTPUT_DIR="${OUTPUT_DIR:-experiments/runs/${RUN_TIMESTAMP}_qwen35_4b_retry_safe_end_v8_screen40_seed42}"
    ;;
  *)
...
if [[ ! -f "${SUPPORT_DECISION}" || ! -f "${STEP_DATA}" || ! -f "${STEP_MANIFEST}" ]]; then
  echo "Missing V8 support decision or frozen optimizer data" >&2
  exit 1
fi

.venv/bin/python - "${SUPPORT_DECISION}" "${STEP_MANIFEST}" <<'PY'
import json
import sys
...
if decision.get("status") != "pass" or decision.get("optimizer_authorized") is not True:
    raise SystemExit("V8 support gate does not authorize optimizer steps")
if set(decision.get("optimizer_scenarios") or []) != expected:
    raise SystemExit("V8 support decision does not authorize the all-or-none scope")
if manifest.get("rows") != 144 or set(manifest.get("accepted_scenarios") or []) != expected:
    raise SystemExit("V8 optimizer manifest does not match the preregistration")
PY
...
export OUTPUT_DIR
export STEP_DATA
export EXPECTED_DATASET_ID="planner_retry_safe_end_residual_v8"
...
export WANDB_RUN_GROUP="planner-retry-safe-end-residual-v8"
export WANDB_TAGS="stage2,grpo,planner-v8,retry-safe-end,${RUN_MODE},seed42"
