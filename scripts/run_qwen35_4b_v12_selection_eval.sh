...
SCREEN_DIR="${SCREEN_DIR:?set SCREEN_DIR to the completed V12 screen directory}"
OUT_DIR="${OUT_DIR:-experiments/runs/20260717_qwen35_4b_v12_selection_dev}"
PYTHON_BIN="${PYTHON_BIN:-${ROOT_DIR}/.venv-qwen35-grpo/bin/python}"
CASES="training/planner_grpo_seed_v1/cases/planner_retry_optimizer_matched_v12_selection_dev_cases.jsonl"
...
[[ "$(jq -r '.status' "${SCREEN_AUDIT}")" == "pass" ]] || { echo "V12 screen audit did not pass" >&2; exit 1; }
...
for index in 0 1 2 3; do
  ...
  CUDA_VISIBLE_DEVICES="${GPUS[$index]}" "${PYTHON_BIN}" \
    ...
    --cases "${CASES}" --out-dir "${run_dir}" --report-prefix "v12_${label}" \
...
[[ "${status}" -eq 0 ]] || { echo "V12 selection rollout failed" >&2; exit 1; }
...
"${PYTHON_BIN}" training/planner_grpo_seed_v1/scripts/select_planner_grpo_checkpoint.py \
  ...
  --cases "${CASES}" \
  --sft-predictions "${OUT_DIR}/sft/v12_sft_run1_predictions.jsonl" \
  --candidate "checkpoint-2=${OUT_DIR}/checkpoint-2/v12_checkpoint-2_run1_predictions.jsonl" \
  --candidate "checkpoint-5=${OUT_DIR}/checkpoint-5/v12_checkpoint-5_run1_predictions.jsonl" \
  --candidate "checkpoint-8=${OUT_DIR}/checkpoint-8/v12_checkpoint-8_run1_predictions.jsonl" \
